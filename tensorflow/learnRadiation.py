...
trainEndDate=[2016, 8, 1]
dataFolder='../data/-0.06-39.99/'

validationInitDate = [2015, 1, 1]
validationEndDate = [2015, 8, 1]

# Import data
dataset=loadData.loadRadiationData(dataFolder,trainInitDate,trainEndDate,windowSize)
...
# Test trained model
valSet=loadData.loadRadiationData(dataFolder,validationInitDate,validationEndDate,windowSize)
result=np.empty((0,1))
labels=np.empty((0,1))
while(valSet.epochs_completed!=1):
  batch_xs, batch_ys = valSet.next_Valbatch(50)
  batch_ys=batch_ys.reshape((50,1))
